...
const updateAddToCartProduct=asyncHandler(async(req,res)=>{
    ...
    const updateProduct=await Cart.updateOne({_id:addToCartProductId},{...(qty && {quantity:qty})})
...
const getAllUsers=asyncHandler(async(req,res)=>{
    ...
    res.status(200).json(new ApiResponse(200,allUsers,'All Users Fetched Successfully'))
})
...
const addToCartViewProduct=asyncHandler(async(req,res)=>{
    ...
    res.status(200).json(200,allProduct,'Add to cart view products fetched')
})
...
const addToCartController=asyncHandler(async(req,res)=>{
    ...
    if(isProductAvailable){
        return res.status(400).json(new ApiResponse(400,{},'Already Exists in Cart'))
    }
    ...
    return res.status(200).json(new ApiResponse(200,saveProduct,'Product added in cart'))
})

export {userSignUp,userSignIn, userLogout,getUserDetails,updateUser,updateAddToCartProduct, deleteAddToCartProduct, countAddToCartProduct, getAllUsers, addToCartViewProduct, addToCartController}